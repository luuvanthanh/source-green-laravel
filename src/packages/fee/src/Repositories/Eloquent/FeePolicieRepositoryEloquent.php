...
class FeePolicieRepositoryEloquent extends CoreRepositoryEloquent implements FeePolicieRepository
{
    ...
    public function calculatorCaseOne($listMonthAge, $fee, $paymentForm, $feePolicie)
    {
        switch ($paymentForm->Code) {
            ...
            case self::YEAR:
                ...
                break;
            case self::SEMESTER1_SEMESTER2:
                $result = $this->calculatorMoneyFeeAndMoneyMealBySemester($listMonthAge, $fee, $paymentForm, $feePolicie);
                break;
            default:
                # code...
                break;
        ...
        foreach ($listMonthAge['detailStudent'] as $monthAge) {
            ...
            if ($fee->Type == self::MEAL_FEE) {
                $dataMealFee = [
                    'classType' => $monthAge['classType'],
                    'month' => $monthAge['month'],
                    'fee' => !isset($dataMealFee) ? $totalMealFee : 0,
                    'content' => $feeDetail->Content
                ];
                $result['dataMealFee'][] = $dataMealFee;
            }
    ...
    public function calculatorMoneyFeeAndMoneyMealBySemester($listMonthAge, $fee, $paymentForm, $feePolicie)
    {
        $totalFeehk1 = 0;
        $totalFeehk2 = 0;
        $totalMealFee = 0;
        foreach ($listMonthAge['dataPaymentForm'] as $dataPaymentForm) {
            ...
            foreach ($dataPaymentForm['dataClassType'] as $key => $value) {
                if ($dataPaymentForm['paymentFormCode'] == self::SEMESTER1) {
                    ...
                    $totalFeehk1 += ($feeDetail->OldStudent * $value['numberClassType']) / $dataPaymentForm['numberMonthInSemester'];
                }

                if ($dataPaymentForm['paymentFormCode'] == self::SEMESTER2) {
                    $feeDetail = FeeDetail::where('FeePoliceId', $feePolicie->Id)->where('PaymentFormId', $dataPaymentForm['paymentFormId'])->where('ClassTypeId', $value['classTypeId'])->first();
                    $totalFeehk2 += ($feeDetail->OldStudent * $value['numberClassType']) / $dataPaymentForm['numberMonthInSemester'];
                }
            }
        }

        dd($totalFeehk1, $totalFeehk2);
    }
}
