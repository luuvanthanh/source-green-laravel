...
class FeePolicieRepositoryEloquent extends CoreRepositoryEloquent implements FeePolicieRepository
{
    ...
    //tính tiền học phí và tiền ăn của học sinh theo năm
    public function calculatorMoneyFeeAndMoneyMealByYearCaseTwo($listMonthAge, $fees, $paymentForm, $feePolicie, $allDateOfSchoolYear, $isWeekend, $locationWeekOfTheMonth, $dayAdmission)
    {
        foreach ($allDateOfSchoolYear as $key => $value) {
            if ($dayAdmission <= $key) {
                $dateOfMonth[] = $value;
