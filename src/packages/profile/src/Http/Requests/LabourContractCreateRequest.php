            ...
            'contractDate' => [
                ...
                function ($attribute, $value, $fail) {
                    ...
                    if (!is_null($labourContract) && $value <= $labourContract->ContractDate->format('Y-m-d')) {
                        return $fail("Ngày hợp đồng phải lớn hơn ngày hiệu lực hợp đồng gần nhất " . $labourContract->ContractDate->format('d-m-Y'));
                    }
                },
            ],
            ...
            'contractFrom' => ['required', 'date',
                function ($attribute, $value, $fail) {
                    ...
                    if (!is_null($labourContract) && $value <= $labourContract->ContractTo->format('Y-m-d')) {
                        return $fail("Thời hạn từ phải lớn hơn thời hạn đến hợp đồng gần nhất " . $labourContract->ContractTo->format('d-m-Y'));
                    }
                },
            ],
