...
use GGPHP\YoungAttendance\ShiftSchedule\Services\ScheduleExceptionService;
use GGPHP\YoungAttendance\ShiftSchedule\Services\ScheduleRepeatService;
...
class ScheduleRepositoryEloquent extends CoreRepositoryEloquent implements ScheduleRepository
{
    ...
    public function __construct(StudentRepositoryEloquent $studentRepositoryEloquent, Application $app)
    {
    ...
    public function createOrUpdate(array $attributes)
    {
        ...
        if (isset($attributes['repeatBy'])) {
            ...
            ScheduleRepeatService::add(\Arr::except($attributes, ['StartDate', 'StudentId', 'ShiftId']));
        }
    ...
    public function scheduleUser(array $attributes)
    {
        ...
        if (!empty($attributes['limit'])) {
            ...
            $scheduleUser = $this->studentRepositoryEloquent->get();
        }
    ...
    public function deleteAll($id, array $attributes)
    {
        ...
        $oldschedule = $this->model()::whereNotIn('Id', [$id])->where(function ($query) use ($attributes, $dateEndYear) {
            $query->where([['EndDate', '>=', $attributes['startDate']], ['EndDate', '<=', $dateEndYear]]);
        })->where('ShiftId', $schedule->ShiftId)->where('StudentId', $schedule->StudentId)->get();
