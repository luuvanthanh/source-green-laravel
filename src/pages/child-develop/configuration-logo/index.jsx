...
const General = memo(() => {
  ...
  const loadingSubmit = effects[`childDevelopConfigurationLogo/ADD`];
  const loading = effects[`childDevelopConfigurationLogo/GET_DETAILS`];
            ...
            <Loading loading={loading} isError={error.isError}>
              <div style={{ padding: 20 }} className="pb-0 border-bottom">
                ...
                <div className={stylesModule['wrapper-modal']}>
                  <div className={stylesModule.img}>
                    {files?.length <= 0 && (
                      <MultipleImageUpload files={files} callback={(files) => uploadFiles(files)} />
                    )}
                    {files?.length > 0 && (
                      <CustomListUpload
                        data={files}
                  ...
                  <div className="pl15 d-flex align-items-center pb20">
                    <p className={stylesModule['text-uploadImg']}>
                      Áp dụng đối với định dạng jpg, png, jpeg, webp. Dung lượng tối đa không quá
                      20mb
                    </p>
                  </div>
                </div>
              ...
              <div className={stylesModule['wrapper-btn']}>
                <Button
                  color="success"
                  size="large"
                  permission={`${FLATFORM.WEB}${permissions.SPTCT_CAUHINHCHENLOGO}${ACTION.EDIT}`}
                  htmlType="submit"
