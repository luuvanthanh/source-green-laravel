...
const { confirm } = Modal;
const mapStateToProps = ({ allocationChildren, loading }) => ({
  data: allocationChildren.data,
  pagination: allocationChildren.pagination,
  loading,
});
...
class Index extends PureComponent {
  ...
  onLoad = () => {
    ...
    this.props.dispatch({
      type: 'allocationChildren/GET_DATA',
      payload: {
        ...search,
        status,
      },
  ...
  onRemove = (id) => {
    ...
    confirm({
      ...
      onOk() {
        dispatch({
          type: 'allocationChildren/REMOVE',
          payload: {
            id,
            pagination: {
              limit: 10,
  ...
  render() {
    ...
    const loading = effects['allocationChildren/GET_DATA'];
    return (
      <>
        <Helmet title="Lịch sử phân bổ" />
        <div
          className={classnames(styles['content-form'], styles['content-form-allocationChildren'])}
        >
          {/* FORM SEARCH */}
