...
const mapStateToProps = ({ criteriaEnglish, loading, user }) => ({
  loading,
  data: criteriaEnglish.data,
  error: criteriaEnglish.error,
  classes: criteriaEnglish.classes,
  branches: criteriaEnglish.branches,
  pagination: criteriaEnglish.pagination,
  students: criteriaEnglish.students,
  subject: criteriaEnglish.subject,
...
class Index extends PureComponent {
  ...
  onLoad = () => {
    ...
    const idSubject = subject?.find(i => i?.name === "Tiếng Anh");
    this.props.dispatch({
      type: 'criteriaEnglish/GET_DATA',
      payload: {
  ...
  loadCategories = () => {
    ...
    if (search?.BranchId) {
      dispatch({
        type: 'criteriaEnglish/GET_CLASSES',
        payload: {
    ...
    if (search?.ClassId) {
      dispatch({
        type: 'criteriaEnglish/GET_STUDENTS',
        payload: {
    ...
    dispatch({
      type: 'criteriaEnglish/GET_BRANCHES',
      payload: {},
    });
    dispatch({
      type: 'criteriaEnglish/GET_SUBJECT',
      payload: {},
      callback: (res) => {
        if (res) {
          if (search?.BranchId && search?.StudentId && search?.ClassId && res?.length > 0) {
            this.setStateData({ loadData: true });
            const idSubject = res?.find(i => i?.name === "Tiếng Anh");
            this.props.dispatch({
              type: 'criteriaEnglish/GET_DATA',
              payload: {
                BranchId: search?.BranchId,
  ...
  render() {
    ...
    const loading = effects['criteriaEnglish/GET_DATA'];
    return (
      <>
        <Helmet title="Tiếng Anh" />
        <div className={classnames(styles['content-form'], styles['content-form-children'])}>
          {/* FORM SEARCH */}
          <div className="d-flex justify-content-between align-items-center mt-3 mb-3">
            <Text color="dark">Tiếng Anh</Text>
          </div>
