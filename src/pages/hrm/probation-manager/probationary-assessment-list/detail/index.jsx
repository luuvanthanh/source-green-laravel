...
const Index = memo(() => {
  ...
  const {
    loading: { effects },
    menuLeftHRM,
  } = useSelector(({ menu, loading, hrmProbationManagerProbationaryAssessmentListAdd }) => ({
    loading,
    menuLeftHRM: menu.menuLeftHRM,
    error: hrmProbationManagerProbationaryAssessmentListAdd.error,
  }));

  const loadingSubmit = effects[`hrmProbationManagerProbationaryAssessmentListAdd/UPDATE`] || effects[`hrmProbationManagerProbationaryAssessmentListAdd/ADD`];

  const onFinish = () => {
    form.validateFields().then((values) => {
      dispatch({
        type: params.id ? 'hrmProbationManagerProbationaryAssessmentListAdd/UPDATE' : 'hrmProbationManagerProbationaryAssessmentListAdd/ADD',
        payload: {
          id: params.id,
          name: values?.name,
  ...
  useEffect(() => {
    if (params.id) {
      dispatch({
        type: 'hrmProbationManagerProbationaryAssessmentListAdd/GET_DATA',
        payload: params,
        callback: (response) => {
          if (response) {
            form.setFieldsValue({
          ...
          <Pane >
            <Form layout="vertical" onFinish={onFinish} form={form} initialValues={{}}>
              <Loading
                ...
                loading={effects['hrmProbationManagerProbationaryAssessmentListAdd/GET_DATA']}
              >
                <Pane className="card p20"  >
                  ...
                  <Pane className="row">
                    ...
                    <Pane className="col-lg-9">
                      <FormDetail name="Phỏng vấn kinh doanh" label="Nội dung đánh giá" type={variables.TYPE.TEXT} />
                    </Pane>
                    <Pane className="col-lg-3">
                      <FormDetail name="Nguyễn Thị Linh" label="Tên nhân sự" type={variables.TYPE.TEXT} />
                    </Pane>
                    <Pane className="col-lg-3">
                      <FormDetail name="Kinh doanh" label="Bộ phận" type={variables.TYPE.TEXT} />
                    </Pane>
                    <Pane className="col-lg-3">
                      <FormDetail name="Kinh doanh" label="Cấu hình áp dụng" type={variables.TYPE.TEXT} />
                    </Pane>
                    <Pane className="col-lg-3">
