...
const Index = memo(({ classId, branchId }) => {
  ...
  const [{ notes }, loading] = useSelector(({ loading: { effects }, warning }) => [
    warning,
    effects,
  ]);
      ...
      <div className={classnames(styles['block-category'])}>
        <div className={styles['body-tab']}>
          <div className={styles['header-tab']}>
            ...
            <p className={classnames('mb0', 'font-size-14')}>{notes?.totalCount || 0}</p>
          </div>
          <Scrollbars autoHeight autoHeightMax={window.innerHeight - 355}>
            <div className="px20">
              {!loading['warning/GET_DATA_NOTE'] && _.isEmpty(notes) && (
                <div className="p20">
                  <Empty />
                </div>
              )}
              {loading['warning/GET_DATA_NOTE'] ? (
                <div className="mt20">
