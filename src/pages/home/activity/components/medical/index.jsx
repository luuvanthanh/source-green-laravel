...
const Index = memo(({ classId }) => {
  ...
  const [{ medicals }, loading] = useSelector(({ loading: { effects }, activity }) => [
    activity,
    effects,
  ]);
  ...
  const fetchData = () => {
    dispatch({
      type: 'activity/GET_DATA_MEDICAL',
      payload: {
        ...search,
      ...
      <div className={classnames(styles['block-category'])}>
        <div className={styles['body-tab']}>
          <div className={styles['header-tab']}>
            ...
            <p className={classnames('mb0', 'font-size-14')}>{medicals?.totalActivity || 0}</p>
          </div>
          <Scrollbars autoHeight autoHeightMax={window.innerHeight - 355}>
            <div className="px20">
              {!loading['activity/GET_DATA_MEDICAL'] && _.isEmpty(medicals) && (
                <div className="p20">
                  <Empty />
                </div>
              )}
              {loading['activity/GET_DATA_MEDICAL'] ? (
                <div className="mt20">
                ...
                <Timeline className={classnames(styles['timeline-container'], 'mt20')}>
                  {medicals?.changeLogs?.map((item, index) => (
                        ...
                        <div key={indexChange} className="mt5 mb5">
                          ...
                          {itemChange.note === 'IsDrunk' && (
                            <p className={styles.norm}>
                              Đã cho uống thuốc {itemChange?.medicine?.name}
                            </p>
                          )}
                          {itemChange.note === 'IsReceived' && (
                            <p className={styles.norm}>
                              Đã nhận thuốc {itemChange?.medicine?.name}
                            </p>
                          )}
                        </div>
                      ))}
