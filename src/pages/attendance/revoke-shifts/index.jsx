...
class Index extends PureComponent {
  ...
  header = () => {
    ...
    return [
      ...
      },
      {
        ...
        width: 120,
        render: (record) => Helper.getDate(record.date_violation, variables.DATE_FORMAT.DATE),
      },
      {
        title: 'Ca làm việc',
        key: 'shift',
        className: 'min-width-150',
        width: 150,
        render: (record) => get(record, 'shift.shift_code'),
      },
      {
        title: 'Thời gian ca',
        key: 'time',
        ...
        render: (record) =>
          record?.shift?.shiftDetail.map((item) => {
            const startTime = moment(item.start_time, variables.DATE_FORMAT.TIME_FULL);
            const endTime = moment(item.end_time, variables.DATE_FORMAT.TIME_FULL);
            return (
              <span key={item.id}>
  ...
  render() {
    ...
    return (
      <>
        <Helmet title="Danh sách bỏ ca" />
        <div className={classnames(styles['content-form'], styles['content-form-revokeShifts'])}>
          {/* FORM SEARCH */}
          <div className="d-flex justify-content-between align-items-center mt-3 mb-3">
            <Text color="dark">Danh sách bỏ ca</Text>
          </div>
