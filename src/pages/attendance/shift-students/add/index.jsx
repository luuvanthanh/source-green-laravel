...
const mapStateToProps = ({ shiftStudentsAdd, loading, menu }) => ({
  ...
  details: shiftStudentsAdd.details,
});
...
class Index extends PureComponent {
  ...
  componentDidMount() {
    ...
    if (get(params, 'id')) {
      dispatch({
        type: 'shiftStudentsAdd/GET_DETAILS',
        payload: get(params, 'id'),
      });
    }
    dispatch({
      type: 'shiftStudentsAdd/GET_BRANCHES',
      payload: get(params, 'id'),
    });
  }
  ...
  onFinish = (values) => {
    ...
    dispatch({
      type: get(params, 'id') ? 'shiftStudentsAdd/UPDATE' : 'shiftStudentsAdd/ADD',
      payload: {
        ...values,
        id: get(params, 'id'),
  ...
  render() {
    ...
    const loading = effects['shiftStudentsAdd/GET_DETAILS'];
    const loadingSubmit = effects['shiftStudentsAdd/ADD'] || effects['shiftStudentsAdd/UPDATE'];
    return (
      <>
        <Breadcrumbs
          last={params.id ? 'Chỉnh sửa mới lịch học' : 'Tạo mới lịch học'}
          menu={menuData}
        />
        <Form
          ...
          ref={this.formRef}
        >
          <Loading loading={loading} isError={error.isError} params={{ error, goBack: '/diem-danh/cau-hinh-lich-hoc' }}>
