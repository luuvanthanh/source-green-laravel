...
const Index = memo(() => {
  ...
  const loadingSubmit = effects[`physicalPeriodicMeasurementConfirmed/ADD`];
  const onFinish = () => {
    setCheckFinish(true);
    ...
    if (checkEmpty?.length === 0) {
      console.log("check");
      dispatch({
        type: 'physicalPeriodicMeasurementConfirmed/ADD',
        payload: {
          id: params?.id,
  ...
  const addApprove = () => {
    dispatch({
      type: 'physicalPeriodicMeasurementConfirmed/APPROVE',
      payload: [params?.id],
      callback: (response, error) => {
        if (response) {
          history.goBack();
