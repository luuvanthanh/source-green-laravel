...
const Index = memo(() => {
  ...
  const [
    ...
    loading,
  ] = useSelector(({ loading: { effects }, criteriaAngleTool }) => [criteriaAngleTool, effects]);
      ...
      render: (record) => (
        <div className={styles['list-button']}>
          <Button
            ...
            permission={`${FLATFORM.WEB}${permissions.CTH_HOCTAPGIAOCU_GOCGIAOCU}${ACTION.EDIT}`}
          >
            Chi tiết
          </Button>
        </div>
  ...
  useEffect(() => {
    dispatch({
      type: 'criteriaAngleTool/GET_DATA',
      payload: { ...search },
    });
  }, [search]);
  ...
  return (
    <>
      <Helmet title="Danh sách góc giáo cụ" />
      <Pane className="p20">
        <Pane className="d-flex mb20">
          <Heading type="page-title">Danh sách góc giáo cụ</Heading>
          <Button
            className="ml-auto"
            color="success"
            icon="plus"
            onClick={() => history.push(`${pathname}/them-moi`)}
            permission={`${FLATFORM.WEB}${permissions.CTH_HOCTAPGIAOCU_GOCGIAOCU}${ACTION.CREATE}`}
          >
            Tạo mới
          </Button>
        </Pane>
