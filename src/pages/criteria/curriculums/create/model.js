...
export default {
  ...
  reducers: {
    ...
    SET_CURRICULUM_TEMPLATES: (state, { payload }) => ({
      ...state,
      curriculumTemplates: payload.items,
    }),
  ...
  effects: {
    ...
    *GET_CURRICULUM_TEMPLATES({ payload }, saga) {
      try {
        const response = yield saga.call(categories.getCurriculumTemplates, payload);
        yield saga.put({
          type: 'SET_CURRICULUM_TEMPLATES',
          payload: response,
        });
      } catch (error) {
        yield saga.put({
