...
const mapStateToProps = ({ curriculums, loading }) => ({
  loading,
  error: curriculums.error,
  data: curriculums.data,
  pagination: curriculums.pagination,
});
...
class Index extends PureComponent {
  ...
  render() {
    ...
    const { search } = this.state;
    const loading = effects['curriculums/GET_DATA'];
    return (
      <>
              ...
              onRow={(record) => ({
                onClick: () => {
                  if (ability.can('CTH', 'CTH')) {
                    history.push(`/chuong-trinh-hoc/${record.id}/chi-tiet`);
                  }
                },
              })}
