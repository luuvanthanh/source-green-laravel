...
const Index = memo(
  ({
    ...
    const loadingSubmit =
      effects[`timeTablesScheduleAdd/ADD`] || effects['timeTablesScheduleAdd/UPDATE'];
    ...
    const onFinish = (values) => {
      const studentIds = isAllStudent
        ? []
        : students?.filter((item) => item.checked)?.map((item) => item?.student?.id);

      const startTime = !isEmpty(values?.rangeTime)
        ? Helper.getDate(values.rangeTime[0], variables.DATE_FORMAT.TIME_FULL)
        : null;
      const endTime = !isEmpty(values?.rangeTime)
        ? Helper.getDate(values.rangeTime[1], variables.DATE_FORMAT.TIME_FULL)
        : null;
      const payload = {
        ...values,
        isReminded,
        ...
        studentIds,
        excludedStudentIds: isAllStudent
          ? students?.filter((item) => !item.checked)?.map((item) => item?.student?.id)
          : [],
    ...
    const onChangeClass = (value) => {
      getStudents({
        ...
        hasMore: true,
        loading: false,
        branchId: searchStudents?.branchId,
        class: value,
      });
    };
    ...
    useEffect(() => {
      if (params?.id) {
        dispatch({
          ...
          callback: (res) => {
            if (res) {
              const classes = !isEmpty(res?.classTimetables)
                ? res?.classTimetables?.map((item) => item?.class?.id)
                : [];
              formRef?.current?.setFieldsValue({
                ...res,
                applyDate: res?.applyDate ? moment(res?.applyDate) : null,
                scheduleSendingDate: res?.scheduleSendingDate
                  ? moment(res?.scheduleSendingDate)
                  : null,
                rangeTime:
                  ...
                  {params?.id ? (
                    <Pane className="card" style={{ padding: 20 }}>
                      ...
                      <Pane className={csx('row', 'border-bottom')}>
                        ...
                        <Pane className="col-lg-6">
                          <FormDetail
                            name={!detail?.classId ? 'Tất cả các lớp' : detail?.classId}
                            label="Lớp"
                            data={classes}
                            type={!detail?.classId ? variables.TYPE.TEXT : variables.TYPE.SELECT}
                          />
                        </Pane>
                      </Pane>
                    ...
                    <Pane className="card" style={{ padding: 20 }}>
                                  ...
                                  <ListItem key={student?.id} className={styles.listItem}>
                                    <Pane className="w-100 d-flex align-items-center">
                                      ...
                                      <Pane className={styles.userInformation}>
                                        <AvatarTable
                                          fileImage={Helper.getPathAvatarJson(student?.fileImage)}
                                        />
                                        <Pane>
                                          <div className="d-flex">
                                            <h3>{student?.fullName}</h3>
                                            {!hasParentAccount && (
                                              <p className="text-danger ml5">(Chưa có tài khoản)</p>
                                            )}
                                          </div>
                      ...
                      {!isAllStudent &&
                        errorStudent &&
                        size(students.filter((item) => item?.checked)) === 0 && (
                          <span className="text-danger mt5">
                            {variables.RULES.EMPTY_INPUT.message}
                          </span>
                        )}
                    </Pane>
                  )}
                ...
                <Pane className="col-lg-6">
                        ...
                        <Pane className="col-lg-12 mt15 mb15">
                          <div>
                            ...
                            <Upload {...props} className={stylesForm['upload-file']}>
                              ...
                              <i>
                                Chỉ hỗ trợ định dạng .pdf, .docx, .xlsx. Dung lượng không được quá
                                5mb
                              </i>
                            </Upload>
                          </div>
                  ...
                  <Pane className="d-flex justify-content-between align-items-center mb20">
                    ...
                    <Button
                      ...
                      permission={
                        isEmpty(params?.id)
                          ? 'WEB_TKB_LICHLAMVIECSUKIEN_CREATE'
                          : 'WEB_TKB_LICHLAMVIECSUKIEN_EDIT'
                      }
                    >
                      Gửi
                    </Button>
