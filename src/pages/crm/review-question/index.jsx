...
const Index = memo(() => {
  const {
    loading,
  } = useSelector(({ loading, crmReviewQuestion }) => ({
    loading,
    colorTags: crmReviewQuestion.colorTags,
  }));
  ...
  useEffect(() => {
    dispatch({
      type: 'crmReviewQuestion/GET_TAGS',
      payload: {},
      callback: (response) => {
    ...
    <>
      <Pane className="pl20 pr20 mt20">
        <Form ref={formRef} onFinish={onFinish} initialValues={{}} layout="vertical">
          {/* <Loading
            loading={loading['crmReviewQuestion/GET_DATA'] || loading['crmReviewQuestion/GET_UNITS']}
          > */}
          <Pane className="row">
